...
<?php
	...
	if(isset($_SESSION["el_login"]) && $_SESSION["el_login"])
	{
		...
		if(isset($_GET["H_BEN"]))
		{
			// Variables from penceramah.php
			$ID	= $_GET["H_BEN"];
			
			//SQL query command
			$sql="INSERT INTO HIT_BENGKEL (NAMA, ALAMAT, NEGERI, SEKTOR, INDUS, PENCERAMAH, JAWATAN, KELAYAKAN, KEMAHIRAN) SELECT NAMA, ALAMAT, NEGERI, SEKTOR, INDUS, PENCERAMAH, JAWATAN, KELAYAKAN, KEMAHIRAN FROM BENGKEL WHERE ID_BEN = $ID;";
			...
			if ($exe_sql)
			{
				$sql2="DELETE FROM BENGKEL WHERE ID_BEN = $ID;";
				...
				echo '<html>
						<head>
							<script>
								window.alert("Data bagi penceramah berjaya disenarai hitam!");
							</script>
			...
			else
			{
				...
				echo '<html>
						<head>
							<script>
								window.alert("Data bagi penceramah gagal disenarai hitam!");
								window.history.go(-1);
							</script>
						</head>
					</html>';
		...
		if(isset($_GET['PADAM']) && is_numeric($_GET['PADAM']))
		{
			...
			// delete the entry
			$result = mysql_query("DELETE FROM BENGKEL WHERE ID_BEN = $ID"); 
        ...
        <nav class="navbar navbar-default navbar-fixed">
            <div class="container-fluid">
                <div class="navbar-header">
                    ...
                    <a class="navbar-brand" href="../ezloc/penceramah.php">Penceramah</a>
                </div>
            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-md-12">
                        <div class="card">
                            <div class="header">
                                <h4 class="title">Senarai Direktori Penceramah</h4>
                            </div>
...
<?php
	...
	if (isset($_GET['CARI']))
	{
		...
		$sql = "SELECT * FROM BENGKEL WHERE NAMA LIKE '%$CARI%' UNION SELECT * FROM BENGKEL WHERE ALAMAT LIKE '%$CARI%' UNION SELECT * FROM BENGKEL WHERE PENCERAMAH LIKE '%$CARI%' ORDER BY NAMA";
	...
	if((isset($_GET['NEG'])) && (isset($_GET['SEK'])) && (isset($_GET['IND'])))
	{
		...
		// create the listing query
		$sql = "SELECT * FROM BENGKEL $TW ORDER BY NEGERI, NAMA";
...
?>
										<tr>
											...
											<?php if(isset($_SESSION["el_login"])){ echo "<td align=\"center\"><a href=\"pinda_ben.php?ID=$ID_BEN\"><button class=\"btn btn-xs btn-warning btn-fill\">Pinda</button></a><br><br><button class=\"btn btn-xs btn-default btn-fill\" type=\"button\" value=\"Senarai Hitam\" onClick='confirmHIT($ID_BEN)'>Senarai Hitam</button><br><br><button class=\"btn btn-xs btn-danger btn-fill\" type=\"button\" value=\"Padam\" onClick='confirmDel($ID_BEN)'>Padam</button></td>"; } ?>
                                        </tr>
<?php
		}
...
?>
										<tr>
                                        	<td align="center" colspan="9"><i>Sila klik butang "Papar" diatas atau lakukan carian</i></td>
                                        </tr>
<?php
	}
?>
	...
	<script type="text/javascript">
		...
		{
			var del = confirm("Adakah anda pasti untuk memadam data penceramah ini?");
			if (del == true)
			{
				window.location.assign("penceramah.php?PADAM=" + nums);
			} else 
			{
		...
		{
			var hit = confirm("Adakah anda pasti untuk menyenarai hitam data penceramah ini?");
			if (hit == true)
			{
				window.location.assign("penceramah.php?H_BEN=" + num_h);
			} else 
			{
				alert("Data penceramah tidak disenarai hitam.");
			}
		}
	</script>
